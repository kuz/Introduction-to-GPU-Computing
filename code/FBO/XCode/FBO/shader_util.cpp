...
GLuint compile(GLuint type, std::string source) {
    ...
    std::vector<const GLchar *> lines2;
    std::string line;
    std::istringstream ss(source);
    while(getline(ss, line)) {
        line += '\n';
        v_lines.push_back(line);
        //lines2.push_back(line.c_str());
        lines2.push_back(v_lines.back().c_str());
    }
    
    for (int i = 0; i < lines2.size(); i++) {
        //std::cout << "-------- " << lines2[i].size() << "------------ " << std::endl;
        std::cout << lines2[i];
    }
    std::cout << "==========" << std::endl;
    ...
    // Compile the source
    glShaderSource(shader, lines.size(), (const char**)&lines[0], NULL);
    glCompileShader(shader);
